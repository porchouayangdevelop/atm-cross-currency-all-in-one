      ...
      QueryAccName: async function (args: any) {
        try {
          ...
          if (isDisallowedCurrency(currency.fromCurrency)) {
            return {
              QueryAccNameResult:
                getValidateFailedMessage("10") +
                `, must be one of [${AllowedCCY}]`,
            };
          }
          ...
          switch (isAllowedCurrency(currency.fromCurrency)) {
            case true:
              ...
              if (isValidateTargetAccountLength(currency.targetCurrency)) {
                return {
                  QueryAccNameResult:
                    getValidateFailedMessage("09") +
                    `, must be one of [${AllowedCCY}]`,
                };
              }

              const deposit: DepositItems | any = await depositRepo(
                ATMID,
                AccNum
              );
							console.log(`-`.repeat(50));
              console.log(
                `You're use deposit case. processRequest : ${JSON.stringify(
                  processRequest
                )} `
              );
              console.log(`Process request: ${JSON.stringify(processRequest)}`);
              console.log(
                `From currency ${deposit.from_ccy} target to ${deposit.to_ccy}`
              );

              // console.log(`-`.repeat(50));

              if (
                deposit.currency_name === "ACCOUNT_NOT_FOUND" &&
              ...
              const isValidStatus = await validateStatusRepostiry(
                currency.targetCurrency
              );

              if (isValidStatus == "12") {
                return {
                  QueryAccNameResult: `The target account or currency status ${getValidateAccountMapping(
                    isValidStatus
                  )}, can not do transaction`,
                };
              }

              if (isValidStatus == "16") {
                console.log(
                  `The target account or currency status ${getValidateAccountMapping(
                    isValidStatus
                  )}, can not do transaction									`
                );
              ...
              const response: QueryAccNameResponse = {
                QueryAccNameResult:
                  deposit.is_allowed == "1"
                    ? deposit.name
                    : getValidateFailedMessage("76"),
              };
            ...
            case false:
              ...
              if (isValidSourceAccountLength(account.fromAccount)) {
                return {
                  QueryAccNameResult:
                    getValidateFailedMessage("07") +
                    `, must be one of [${allowedAccountLength}] digits length`,
                };
              }

              if (
                isValidAccountsMatch(account.fromAccount, account.targetAccount)
              ) {
                return {
